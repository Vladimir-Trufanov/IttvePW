 ...
 * 
 * v3.0.5, 28.05.2026                                 Автор:      Труфанов В.Е.
...
// _DS(ST,"AT+CCLK=\"26/05/28,09:51:10+12\"")
...
_DS(AT_CCLK,"AT+CCLK?")      // узнать время часов SIM900
...
void ATerrorMess(char* ATcommand, uint8_t answer) 
{
  ...
  if (answer==1) Serial.println(F("Ответ SIM900 превышает размер буфера")); 
...
Для отправки команда должна быть размещена в буфере response не более bufsize-1 символов с завершающим нулём
Возвращается ответ answer: 
  0 - "передана команда SIM900, получен подтверждающий ответ";
  1 - "ответ SIM900 превышает размер буфера";
...
uint8_t AT_(unsigned int timeout)
{
  ...
  memset(response,'\0',bufsize);  // очистили буфер 
